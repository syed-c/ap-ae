...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
    ...
    if (!cityData) {
        ...
        treatmentMatch = queryClient.getQueryData<any>(['treatment-match', citySlug]);

        if (treatmentMatch) {
            // This is a StateServicePage case
            // Prefetch cities for this state
            await queryClient.prefetchQuery({
                queryKey: ['cities-by-state', normalizedStateSlug],
                queryFn: async () => {
                    const { data } = await supabase
            ...
            // Prefetch State Service Profiles
            await queryClient.prefetchQuery({
