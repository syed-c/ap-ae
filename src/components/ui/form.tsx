...
const FormLabel = React.forwardRef<React.ElementRef<typeof LabelPrimitive.Root>, React.ComponentPropsWithoutRef<typeof LabelPrimitive.Root>>(
  ({ className, ...props }, ref) => {
    ...
    return <Label ref={ref} className={cn(error && "text-[#EF4444]", className)} htmlFor={formItemId} {...props} />;
  },
);
...
const FormDescription = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLParagraphElement>>(({ className, ...props }, ref) => {
  ...
  return <p ref={ref} id={formDescriptionId} className={cn("text-sm text-[#62626B]", className)} {...props} />;
});
...
const FormMessage = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLParagraphElement>>(({ className, children, ...props }, ref) => {
  ...
  if (!body) return null;
  return <p ref={ref} id={formMessageId} className={cn("text-sm font-medium text-[#EF4444]", className)} {...props}>{body}</p>;
});
